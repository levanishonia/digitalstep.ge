...
export function ServiceCard({service,locale}:{service:Service;locale:Locale}) {
 ...
 return <article className="service-card">
  ...
  <div className={`source-badge ${service.serviceSource==='DIGITAL_STEP'?'official':''}`}>{service.serviceSource==='DIGITAL_STEP'?<Sparkles/>:service.providerStatus==='VERIFIED'?<BadgeCheck/>:null}{service.serviceSource==='DIGITAL_STEP'?t.trust.digitalStep:service.providerStatus==='VERIFIED'?t.trust.verified:null}</div>

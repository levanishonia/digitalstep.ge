...
function SectionHeader({ title, action, href }: { title: string; action: string; href: string }) {
  return <div className="section-header"><h2>{title}</h2><a href={href}>{action}<ChevronRight aria-hidden="true" /></a></div>
}

function Services({ id, title, locale, items }: { id: string; title: string; locale: Locale; items: typeof popularServices }) {
  const t = dictionary[locale]
  return <section id={id} className="market-section"><SectionHeader title={title} action={t.home.viewAll} href={`#${id}`} /><div className="service-grid" tabIndex={0}>{items.map(s => <ServiceCard key={s.id} service={s} locale={locale} />)}</div></section>
}
export function MarketplaceHomepage({locale}:{locale:Locale}) { const t=dictionary[locale]
 return <>
  ...
  <section id="categories" className="market-section"><SectionHeader title={t.home.categories} action={t.home.allCategories} href="#categories"/><div className="category-grid">{categories.map(c=><a key={c.id} className="category-card" href="#popular" aria-label={`${t.home.viewCategory}: ${c.name[locale]}`}><span className="category-icon"><MarketplaceIcon name={c.icon}/></span><strong>{c.name[locale]}</strong><small>{c.description[locale]}</small><em>{c.count} {t.home.services}</em></a>)}</div></section>
  ...
  <section id="how-it-works" className="market-section how-section"><SectionHeader title={t.home.works} action={t.home.browse} href="#popular"/><div className="steps">{steps.map((s,i)=><article key={s.id}><b>0{i+1}</b><span><MarketplaceIcon name={s.icon}/></span><h3>{s.title[locale]}</h3><p>{s.description[locale]}</p></article>)}</div></section>

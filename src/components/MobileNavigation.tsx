import { Bell } from 'lucide-react'
import type { Locale } from '../i18n'
import { dictionary, localePath } from '../i18n'
import { Logo } from './Logo'
import { SearchField } from './DesktopHeader'
import { mobileNavigation } from './navigation'
export function MobileHeader({locale}:{locale:Locale}) { const t=dictionary[locale]; const languageHref=localePath(locale==='ka'?'en':'ka',`${window.location.pathname}${window.location.search}${window.location.hash}`); return <header className="mobile-header"><div className="mobile-header-row"><Logo label={`Digital Step — ${t.nav.main}`}/><div className="mobile-tools"><a className="language-switch" href={languageHref}>{t.shell.language}</a><button className="icon-button has-notice" type="button" aria-label={t.shell.notifications}><Bell/></button></div></div><SearchField locale={locale}/></header> }
export function MobileBottomNavigation({locale}:{locale:Locale}) { const t=dictionary[locale]; return <nav className="bottom-navigation" aria-label={t.nav.mobile}>{mobileNavigation.map(({key,href,icon:Icon},index)=>{const center=index===2;return <a key={key} href={href.startsWith('/')?localePath(locale,href):href} className={center?'marketplace-action':undefined} aria-label={t.nav[key]}>{center?<span><Icon/></span>:<Icon/>}<small>{t.nav[key]}</small></a>})}</nav> }

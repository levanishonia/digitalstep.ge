...
export type ApiErrorCode = 'INVALID_CREDENTIALS'|'EMAIL_ALREADY_EXISTS'|'VALIDATION_ERROR'|'UNAUTHENTICATED'|'FORBIDDEN'|'INTERNAL_ERROR'|'RATE_LIMITED'|'NETWORK_ERROR'
...
export const authApi={
  ...
  register:(input:{firstName:string;lastName:string;email:string;phone?:string;password:string;role:'CUSTOMER'|'PROVIDER';preferredLocale:Locale})=>request<{user:AuthUser}>('/api/auth/register',{method:'POST',body:JSON.stringify(input)}),

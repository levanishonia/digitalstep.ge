...
interface CatalogOverride { serviceId:string;status:'DRAFT'|'PENDING_REVIEW'|'ACTIVE'|'ARCHIVED';statusOnly:boolean;isCustom:boolean;titleKa?:string;titleEn?:string;descriptionKa?:string;descriptionEn?:string;category?:string;priceMinor?:number;deliveryDays?:number;isFeatured?:boolean;isPopular?:boolean;packages?:PackageOverride[]|null;slug?:string|null;serviceSource?:'DIGITAL_STEP'|'VERIFIED_PROVIDER';media?:{id:string;type:'IMAGE'|'VIDEO';url:string;width?:number|null;height?:number|null;duration?:number|null;sortOrder:number;isPrimary:boolean}[] }
  ...
  loaded ??= fetch('/api/catalog/overrides')
    ...
    .then(({data}) => {
      for (const override of data.overrides) {
        ...
        if(override.isCustom&&!service&&override.titleKa&&override.titleEn&&override.descriptionKa&&override.descriptionEn&&override.category&&override.priceMinor!==undefined&&override.deliveryDays!==undefined){
        ...
        const media=override.media??[];service.media=[...media].sort((a,b)=>Number(b.isPrimary)-Number(a.isPrimary)||a.sortOrder-b.sortOrder);service.isFeatured=override.isFeatured??false;service.isPopular=override.isPopular??false
        if(media.length)detail.gallery=media.map(item=>({id:item.id,label:{ka:item.type==='IMAGE'?'სერვისის სურათი':'სერვისის ვიდეო',en:item.type==='IMAGE'?'Service image':'Service video'},tone:'media',video:item.type==='VIDEO',url:item.url}))
        if(!override.statusOnly&&override.titleKa&&override.titleEn&&override.descriptionKa&&override.descriptionEn&&override.category&&override.priceMinor!==undefined&&override.deliveryDays!==undefined)Object.assign(service,{title:{ka:override.titleKa,en:override.titleEn},description:{ka:override.descriptionKa,en:override.descriptionEn},category:override.category,price:override.priceMinor/100,deliveryDays:override.deliveryDays,status:override.status})
        ...
        else if(override.priceMinor!==undefined&&override.deliveryDays!==undefined){if(detail.packages[0])Object.assign(detail.packages[0],{price:override.priceMinor/100,deliveryDays:override.deliveryDays});if(orderService.packages[0])Object.assign(orderService.packages[0],{priceMinor:override.priceMinor,deliveryDays:override.deliveryDays})}
      }
    })
  return loaded
}

...
interface AuthContextValue { user:AuthUser|null; loading:boolean; login:(email:string,password:string)=>Promise<AuthUser>; register:(input:{firstName:string;lastName:string;email:string;phone?:string;password:string;role:'CUSTOMER'|'PROVIDER';preferredLocale:Locale;termsAccepted:true})=>Promise<AuthUser>; updateProfile:(input:{firstName?:string;lastName?:string;phone?:string|null;preferredLocale?:Locale})=>Promise<AuthUser>; logout:()=>Promise<void> }

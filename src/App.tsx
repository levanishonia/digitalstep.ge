...
import { MessagesPlaceholder, SettingsPage } from './components/dashboard/SettingsPage'
...
export function App() {
  ...
  if(segments[0]==='dashboard'){
    const page=segments[1]==='orders'&&segments[2]?<OrderDetailPage locale={locale} id={segments[2]}/>:segments[1]==='orders'?<OrdersPage locale={locale}/>:segments[1]==='profile'?<ProfilePage locale={locale}/>:segments[1]==='settings'?<SettingsPage locale={locale}/>:segments[1]==='messages'?<MessagesPlaceholder locale={locale}/>:<DashboardOverview locale={locale}/>
